...
# Function to check if workbook is stale
is_stale() {
  local days=$1
...
for team in "${TEAMS[@]}"; do
  ...
  for workbook in "$TEAM_PATH"/*; do
    if [[ ! -d "$workbook" ]]; then
      continue
    fi

    # Skip archive subdirectory
    TASK_NAME=$(basename "$workbook")
    if [[ "$TASK_NAME" == "archive" ]]; then
      continue
    fi

    DAYS=$(days_since_modified "$workbook")

    # Extract title from README.md
    TITLE=""
    if [[ -f "$workbook/README.md" ]]; then
      TITLE=$(grep '^title:' "$workbook/README.md" 2>/dev/null | sed 's/^title: "\(.*\)"$/\1/' | sed "s/^title: '\(.*\)'$/\1/" | sed 's/^title: \(.*\)$/\1/')
    fi
    if [[ -z "$TITLE" ]]; then
      TITLE="(no title)"
    fi
...
if [[ "$STALE_ONLY" == false ]]; then
  echo ""
  echo "Summary:"
  echo "  Active workbooks: $ACTIVE_COUNT"
  echo "  Stale workbooks: $STALE_COUNT"
fi

if [[ "$STALE_ONLY" == true ]] && [[ $STALE_COUNT -eq 0 ]]; then
  echo "No stale workbooks found"
fi